...
pair<int, int> Engine::get_engine_move(int cboard[8][8], int color[8][8]){
    ...
    for(int i=0; i<moves.captures.size(); i++){
        v = Search::alphabeta(Generator::sim_board(b, moves.captures[i], 2), SEARCH_DEEPNESS,INT_MIN, INT_MAX, 1);
        if(v < move_value){
            move_value = v;
            best =  i; c = 1;
        }
    ...
    for(int i=0; i<moves.moves.size(); i++){
        v = Search::alphabeta(Generator::sim_board(b, moves.moves[i], 2), SEARCH_DEEPNESS,INT_MIN, INT_MAX, 1);
        if(v < move_value){
            move_value = v;
            best =  i; c = 0;
        }
    ...
    logger.log("Engine move calculated!");
...
int Search::alphabeta(board bd, int depth, int alpha, int beta, bool maximize){
    ...
    */
    ...
    if(maximize){
        ...
        for(auto it = moves.captures.begin(); it!=moves.captures.end(); it++){
            value = max(value, alphabeta(Generator::sim_board(bd, *it, 1), depth-1, alpha, beta, 0));
            if(value >= beta){
                cutoff = 1;
                break;
            }
        ...
        if(!cutoff)for(auto it = moves.moves.begin(); it!=moves.moves.end(); it++){
            value = max(value, alphabeta(Generator::sim_board(bd, *it, 1), depth-1, alpha, beta, 0));
            if(value >= beta)break;
            alpha = max(alpha, value);
        }
        ...
        //Could use move ordering euristcs here
        for(auto it = moves.captures.begin(); it!=moves.captures.end(); it++){
            value = min(value, alphabeta(Generator::sim_board(bd, *it, 2), depth-1, alpha, beta, 1));
            if(value <= alpha){
                cutoff = 1;
                break;
            }
